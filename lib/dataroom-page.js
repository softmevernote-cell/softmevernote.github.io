        ...
        fetch('files_info.json')
            ...
            .then(data => {
                ...
                searchInput.addEventListener('keyup', () => {
                    ...
                    const scored = items
                        .map(item => {
                            const matchCount = matchKeywords.reduce((count, keyword) => {
                                let matches = 0;
                                matches += countOccurrences(item.filenameSearch, keyword);
                                matches += countOccurrences(item.filenameCollapsed, keyword);
                                matches += countOccurrences(item.filenameStripped, keyword);
                                matches += countOccurrences(item.htmlFileSearch, keyword);
                                matches += countOccurrences(item.htmlFileCollapsed, keyword);
                                matches += countOccurrences(item.htmlFileStripped, keyword);
